...
from data_labeling import labeling_mode
...
for train, test in kfold.split(train_X, decode(Y)) :
    ...
    model.add(complexnn.conv.ComplexConv1D(filters = 128, kernel_size=16, activation = 'relu', input_shape = (1024, 2), padding='same'))
    model.add(layers.Dropout(0.35))
    model.add(complexnn.conv.ComplexConv1D(filters = 64, kernel_size=8, activation = 'relu', padding='same'))
    model.add(layers.Dropout(0.35))
    ...
    print(cm)
    model.save(f"/content/gdrive/MyDrive/train_1/model_{cnt}.keras")